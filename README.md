...
├── ui/
│   ├── __init__.py
│   ├── theme.py   UI 主题：字体、颜色、默认文件夹等 UI 配置
...
media`，`ui.app → dialogs`。

## 要求
- Windows / Linux（脚本会自动适配平台差异）
- Python 3.7+
- 已安装 FFmpeg 与 FFprobe
  - Windows：可在 `media.py` 顶部 `DEFAULT_FFMPEG_PATH` 配置路径；若未配置，脚本会尝试从 PATH 查找
...
2. 编辑 `media.py` 顶部的 `DEFAULT_FFMPEG_PATH` 为你系统中 ffmpeg 的实际路径（Windows），例如：
...
- 脚本已做跨平台处理：`CREATE_NO_WINDOW` 仅 Windows 生效，Linux 下自动忽略；字体与颜色常量位于 `ui/theme.py`（`FONT_*`、`COLOR_*`），默认文件夹等 UI 配置也在该文件，可按平台自行调整。
